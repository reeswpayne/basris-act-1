...
encountertext = "A den of Leg Snakes attacks!" -- Modify as necessary. It will only be read out in the action select screen.
...
possible_attacks = { "legsnake_fireball", "legsnake_leg_bugtest" }
nextwaves = { "legsnake_fireball" }
...
function EncounterStarting()
    ...
    Inventory.SetInventory({ "Gamer Flask","Katana"})
...
function HandleItem(user, targets, itemID, itemData)
    ...
    elseif itemID == "Katana" then
        ...
        BattleDialog({ user.name .. " throws the Katana at " .. targets[1].name .. "!"})
    end
end
