...
AddAct("Flirt", "Seduce the enemy", 0, { "Ben" })
AddAct("Pose", "Show him who's cool!", 5, { "Pichu" })
AddAct("Working?", "Is this working?", 50, { "Pichu" })
...
function HandleCustomCommand(user, command)
    ...
    elseif command == "Working?" then
        ...
        text = {"Is this working?", "[func:SetFaceSprite,Players.Pichu.Mad][voice:v_ralsei]IT'S WORKING!!!", "Poseur smiles and seems to calm down."}
        canspare = true
        SetCYKAnimation("Idle") -- refresh the animation
    end
    BattleDialog(text)
